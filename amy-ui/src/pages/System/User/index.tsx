...
import { ModalForm, PageContainer, ProFormRadio, ProFormSelect, ProFormText, ProFormTextArea, ProFormTreeSelect, ProTable } from '@ant-design/pro-components';
...
function filterEnabled(nodes: TreeOption[]): TreeOption[] {
  return nodes.filter((node) => !node.disabled).map((node) => ({ ...node, children: node.children ? filterEnabled(node.children) : undefined }));
}
...
export default function UserPage() {
  ...
  const openForm = async (record?: UserRecord) => {
    ...
    setEditing(record ? { ...response.data, postIds: response.postIds || [], roleIds: response.roleIds || [], password: '' } : { userName: '', nickName: '', password: initPassword, status: '0' });
    setFormOpen(true);
  };

  const columns: ProColumns<UserRecord>[] = [
    { title: '用户编号', dataIndex: 'userId', search: false, width: 90 },
    { title: '用户名称', dataIndex: 'userName', copyable: true, render: (_, record) => <a onClick={async () => { const response = await getUser(record.userId); setDetail({ ...response.data, postIds: response.postIds, roleIds: response.roleIds }); setPosts(response.posts); setRoles(response.roles); setDetailOpen(true); }}>{record.userName}</a> },
  ...
  return (
    <PageContainer>
      <Row gutter={16} wrap={false}>
        <Col flex="260px">
          <Card title="组织机构" extra={<a onClick={() => void refreshTree()}>刷新</a>} styles={{ body: { padding: 12 } }}>
            <Tree treeData={toTreeData(deptTree)} defaultExpandAll selectedKeys={deptId ? [deptId] : []} onSelect={(keys) => { setDeptId(keys[0] as number | undefined); actionRef.current?.reload(); }} />
          </Card>
        </Col>
        <Col flex="auto">
          <ProTable<UserRecord>
            ...
            toolBarRender={() => [
              <PermissionButton key="add" type="primary" icon={<PlusOutlined />} permission="system:user:add" onClick={() => void openForm()}>新增</PermissionButton>,
              <PermissionButton key="edit" icon={<EditOutlined />} permission="system:user:edit" disabled={selectedKeys.length !== 1} onClick={() => void openForm({ userId: Number(selectedKeys[0]), userName: '', nickName: '' })}>修改</PermissionButton>,
              <PermissionButton key="delete" danger icon={<DeleteOutlined />} permission="system:user:remove" disabled={!selectedKeys.length} onClick={() => modal.confirm({ title: `确认删除用户编号「${selectedKeys.join(',')}」？`, onOk: async () => { await deleteUsers(selectedKeys); setSelectedKeys([]); message.success('删除成功'); actionRef.current?.reload(); } })}>删除</PermissionButton>,
              <Upload key="import" showUploadList={false} action={`${API_BASE_URL}/system/user/importData`} headers={{ Authorization: `Bearer ${document.cookie.match(/Admin-Token=([^;]+)/)?.[1] || ''}` }} onChange={(info) => { if (info.file.status === 'done') { message.success('导入成功'); actionRef.current?.reload(); } }}><PermissionButton icon={<UploadOutlined />} permission="system:user:import">导入</PermissionButton></Upload>,
              <PermissionButton key="export" icon={<DownloadOutlined />} permission="system:user:export" onClick={() => void downloadFile('/system/user/export', { deptId }, `user_${Date.now()}.xlsx`)}>导出</PermissionButton>
            ]}
          />
        </Col>
      </Row>
      ...
      >
        <ProFormText name="nickName" label="用户昵称" colProps={{ span: 12 }} rules={[{ required: true, message: '用户昵称不能为空' }]} />
        <ProFormTreeSelect name="deptId" label="归属部门" colProps={{ span: 12 }} fieldProps={{ treeData: filterEnabled(deptTree), fieldNames: { label: 'label', value: 'id' } }} />
        <ProFormText name="phonenumber" label="手机号码" colProps={{ span: 12 }} rules={[{ pattern: /^1[3-9]\d{9}$/, message: '请输入正确的手机号码' }]} />
        <ProFormText name="email" label="邮箱" colProps={{ span: 12 }} rules={[{ type: 'email', message: '请输入正确的邮箱地址' }]} />
        {!editing?.userId && <ProFormText name="userName" label="用户名称" colProps={{ span: 12 }} rules={[{ required: true }, { min: 2, max: 20 }]} />}
        {!editing?.userId && <ProFormText.Password name="password" label="用户密码" colProps={{ span: 12 }} rules={[{ required: true }, { min: 5, max: 20 }]} />}
        ...
        <ProFormSelect name="postIds" label="岗位" colProps={{ span: 12 }} mode="multiple" options={posts.map((post) => ({ label: post.postName, value: post.postId, disabled: post.status === '1' }))} />
        <ProFormSelect name="roleIds" label="角色" colProps={{ span: 12 }} mode="multiple" options={roles.map((role) => ({ label: role.roleName, value: role.roleId, disabled: role.status === '1' }))} />
      ...
      <Drawer title="用户详情" width={720} open={detailOpen} onClose={() => setDetailOpen(false)}>
        <Descriptions bordered column={2} items={[
          ...
          { key: 'post', label: '岗位', children: posts.filter((item) => detail?.postIds?.includes(item.postId)).map((item) => item.postName).join('、') || '无岗位' },
