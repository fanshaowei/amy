...
export default function RolePage() {
    ...
    const columns: ProColumns<RoleRecord>[] = [
        ...
        {title: '显示顺序', dataIndex: 'roleSort', search: false, width: 100},
        {
            ...
            render: (_, record) => <Switch checked={record.status === '0'} disabled={record.roleId === 1}
                                           onChange={async (checked) => {
                                               await changeRoleStatus(record.roleId!, checked ? '0' : '1');
        ...
        },
        {
            title: '操作', valueType: 'option', width: 260, render: (_, record) => record.roleId === 1 ? null : [
                <PermissionButton key="edit" type="link" size="small" permission="system:role:edit"
                                  onClick={() => void openRoleForm(record)}>修改</PermissionButton>,
                <PermissionButton key="scope" type="link" size="small" permission="system:role:edit"
                                  onClick={() => void openDataScope(record)}>数据权限</PermissionButton>,
                <PermissionButton key="users" type="link" size="small" permission="system:role:edit" onClick={() => {
                    setAuthRole(record);
                    setAuthOpen(true);
                }}>分配用户</PermissionButton>,
                <PermissionButton key="remove" type="link" danger size="small" permission="system:role:remove"
                                  onClick={() => modal.confirm({
                                      title: `确认删除角色「${record.roleName}」？`,
                                      onOk: async () => {
                                          await deleteRoles([record.roleId!]);
    ...
    const userColumns: ProColumns<UserRecord>[] = [
        ...
        {title: '手机', dataIndex: 'phonenumber'},
        {
            ...
            dataIndex: 'status',
            search: false,
            render: (_, record) => record.status === '0' ? '正常' : '停用'
        },
        {title: '创建时间', dataIndex: 'createTime', search: false},
        {
            title: '操作',
            valueType: 'option',
            render: (_, record) => [<PermissionButton key="cancel" type="link" danger permission="system:role:remove"
                                                      onClick={async () => {
                                                          await cancelUserRole({
                                                              roleId: authRole!.roleId!,
                                                              userId: record.userId!
                                                          });
                                                          message.success('取消授权成功');
                                                          authActionRef.current?.reload();
                                                      }}>取消授权</PermissionButton>]
        }
    ];
    ...
    return <PageContainer>
        <ProTable<RoleRecord>
            ...
            request={async ({current, pageSize, ...params}) => {
                const response = await listRoles({...params, pageNum: current, pageSize});
                return {data: response.rows, total: response.total, success: response.code === 200};
            }}
            toolBarRender={() => [
                <PermissionButton key="add" type="primary" icon={<PlusOutlined/>} permission="system:role:add"
                                  onClick={() => void openRoleForm()}>新增</PermissionButton>,
                <PermissionButton key="edit" icon={<EditOutlined/>} permission="system:role:edit"
                                  disabled={selectedKeys.length !== 1} onClick={() => void openRoleForm({
                    roleId: Number(selectedKeys[0]),
                    roleName: '',
                    roleKey: '',
                    roleSort: 0
                })}>修改</PermissionButton>,
                <PermissionButton key="delete" danger icon={<DeleteOutlined/>} permission="system:role:remove"
                                  disabled={!selectedKeys.length} onClick={() => modal.confirm({
                    title: `确认删除角色编号「${selectedKeys.join(',')}」？`,
                    onOk: async () => {
                        await deleteRoles(selectedKeys);
                        setSelectedKeys([]);
                        message.success('删除成功');
                        actionRef.current?.reload();
                    }
                })}>删除</PermissionButton>,
                <PermissionButton key="export" icon={<DownloadOutlined/>} permission="system:role:export"
                                  onClick={() => void downloadFile('/system/role/export', {}, `role_${Date.now()}.xlsx`)}>导出</PermissionButton>
            ]}
        />
        ...
        <ModalForm<RoleRecord> title={editing?.roleId ? '修改角色' : '添加角色'} open={formOpen} initialValues={editing}
                               ...
                               onFinish={async (values) => {
                                   ...
                                   message.success(editing?.roleId ? '修改成功' : '新增成功');
                                   setFormOpen(false);
                                   actionRef.current?.reload();
                                   return true;
                               }}>
            ...
            <ProFormTreeSelect name="menuIds" label="菜单权限" fieldProps={{
                treeData: menuTree,
                treeCheckable: true,
            ...
            <ProFormRadio.Group name="menuCheckStrictly" label="父子联动"
                                options={[{label: '联动', value: true}, {label: '独立', value: false}]}/>
        ...
        <ModalForm<RoleRecord> title="分配数据权限" open={dataScopeOpen} initialValues={editing}
            ...
            <ProFormSelect name="dataScope" label="权限范围" options={[{label: '全部数据权限', value: '1'}, {
                label: '自定数据权限',
                value: '2'
            ...
            <ProFormDependency name={['dataScope']}>
                {({dataScope}) => dataScope === '2' ? (<>
                    <ProFormTreeSelect name="deptIds" label="数据权限" fieldProps={{
                        treeData: deptTree,
                        treeCheckable: true,
                        treeDefaultExpandAll: true,
                        showCheckedStrategy: 'SHOW_ALL',
                    ...
                    <ProFormRadio.Group name="deptCheckStrictly" label="父子联动"
                                        options={[{label: '联动', value: true}, {label: '独立', value: false}]}/>
                </>) : null}
            </ProFormDependency>
        </ModalForm>
        ...
        <Drawer title={`分配用户 - ${authRole?.roleName || ''}`} width="88%" open={authOpen}
            ...
            <ProTable<UserRecord> rowKey="userId" actionRef={authActionRef} columns={userColumns}
                                  ...
                                  request={async ({current, pageSize, ...params}) => {
                                      const response = await listAllocatedUsers({
                                          ...params,
                                          roleId: authRole?.roleId,
                                          pageNum: current,
                                          pageSize
                ...
                <PermissionButton key="add" type="primary" icon={<UserAddOutlined/>} permission="system:role:add"
                                  onClick={() => setSelectOpen(true)}>添加用户</PermissionButton>,
                <PermissionButton key="cancel" danger permission="system:role:remove" disabled={!authUserIds.length}
                                  onClick={async () => {
                                      await cancelUserRoles({
                                          roleId: authRole!.roleId!,
                                          userIds: authUserIds.join(',')
                                      });
                                      setAuthUserIds([]);
                                      message.success('取消授权成功');
                                      authActionRef.current?.reload();
                                  }}>批量取消授权</PermissionButton>
            ]}/>
        </Drawer>

        <Drawer title="选择用户" width="75%" open={selectOpen} onClose={() => setSelectOpen(false)}
                extra={<Space><PermissionButton type="primary" permission="system:role:add"
                                                disabled={!candidateIds.length} onClick={async () => {
                    await selectRoleUsers({roleId: authRole!.roleId!, userIds: candidateIds.join(',')});
                    setCandidateIds([]);
                    setSelectOpen(false);
                    message.success('授权成功');
                    authActionRef.current?.reload();
                }}>确认授权</PermissionButton></Space>}>
            <ProTable<UserRecord> rowKey="userId" actionRef={selectActionRef}
                                  columns={userColumns.filter((column) => column.dataIndex !== 'createTime' && column.valueType !== 'option')}
                                  rowSelection={{selectedRowKeys: candidateIds, onChange: setCandidateIds}}
                                  request={async ({current, pageSize, ...params}) => {
